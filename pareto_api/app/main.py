...
def evaluate_er_diagram(
    ...
    # ユーザーからの入力をエスケープ処理
    escaped_content = utils.escape_input(diagram_input.content)
    
    # Mermaid ER図をパースしてスコアを計算
    try:
        tables_info = utils.parse_mermaid_er(escaped_content)
        (
    ...
    # パレート効率的な解を計算
    pareto_mask = utils.is_pareto_efficient(np.array(scores))
    pareto_costs = np.array(scores)[pareto_mask]
    pareto_diagrams = [
